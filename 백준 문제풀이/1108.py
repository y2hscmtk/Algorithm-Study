...
'''
A의 점수를 계산할때,
A가 갈수 있는 사이트를 파악하고, 해당 사이트와 해당사이트에서 갈 수 있는 모든 사이트는 A로의 길이 있더라도 고려하지 않는다.

'''

'''
해당 사이트까지 갈수 있는 '길의 개수'를 묻는 문제이므로, 생각한 방법과 다른 알고리즘을 사용해야된다고 판단됨
더 실력이 쌓일때까지 보류하고 나중에 시도해보자
'''
# # n은 50보다 작은 자연수
# n = int(input())
