...
석순의 부딪히는 횟수 처리 => 홀수번에 입력받은 값을 석순 배열에 삽입후 정렬
1,3,5,7,3,3,4,2 가정시 => 1,2,3,3,3,4,5,7 => 같은 값이 여러번 등장할수 있고 우리가 알고 싶은것은 장애물이 부딪히는 횟수
...
종순이 부딪히는 횟수 처리 => 짝수번에 입력받은 값을 H-value 형식으로 종순 배열에 삽입후 정렬
3,4,5,1,2 이라고 가정하고 H = 5일때 => [2,1,0,4,3]이 저장됨 => 정렬시 [0,1,2,3,4]
개똥벌레가 탐색하는 위치가 3번이라고 가정할때 3보다 작은 값에 대해서 모두 부딪힌다.(빼준것이므로) 
