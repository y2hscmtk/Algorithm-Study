...
while True:
    ...
    if count==4: #이동스택이 4라면 => 모든 방향을 다 방문했거나, 바다라면
        ...
        print(game_map[mx][my])
        if game_map[mx][my] != 1 and game_map[mx][my] != 2: #만약 캐릭터의 왼쪽방향이 바다, 혹은 방문한 곳이 아니라면 (방문한 곳은 2로 처리)
            #이동 후 방향 변경
            d = direction[d-1] #방향을 왼쪽방향으로 회전
            #왼쪽방향으로 이동
            x = x + dx[d] 
            y = y + dx[d]
            game_map[x][y] = 2 # 방문한 칸은 2로 처리
            visit+=1 #방문한 칸 횟수 증가
            count = 0 #이동실패 스택 초기화
